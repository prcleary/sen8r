...
multiple_uids <- dept_lookup(c("Microbiology", "Haematology", "Clinical Pathology"),"title", "uid")
```
...
``` r
# Get analysis requests and view the first one
ars <- get_senaite_data('analysisrequest')
...
``` r
# Get report download URLs
arreports <- get_senaite_fields('arreport', fields = 'Pdf')

# View available PDFs
arreports$Pdf.download

# Download a report (filename automatically extracted from URL)
download_senaite_report(url = arreports$Pdf.download[1])
# Creates: SAMPLE-ID.pdf

# Or specify a custom filename
download_senaite_report(
  url = arreports$Pdf.download[1],
